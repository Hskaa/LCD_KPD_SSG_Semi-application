...
void SSG_Mult(SSG *SSG_t){
	 ...
	 ones=(SSG_t->num) %10;
		...
		switch (SSG_t->Copy_u8Port)
		{
		case 1 :
			DIO_u8SetPortDirection(DIO_PORTA,DIO_PORT_HIGH);
			DIO_u8SetPortValue(DIO_PORTA,Arr_DecToSeg[tens]);
			CLR_BIT(PORT_PINS,SSG_t->Copy_u8PinLeft);
			SET_BIT(PORT_PINS,SSG_t->Copy_u8PinRight);
			_delay_ms(10);
			DIO_u8SetPortValue(DIO_PORTA,Arr_DecToSeg[ones]);
			SET_BIT(PORT_PINS,SSG_t->Copy_u8PinLeft);
			CLR_BIT(PORT_PINS,SSG_t->Copy_u8PinRight);
			_delay_ms(10);
			break;
		case 2 :
